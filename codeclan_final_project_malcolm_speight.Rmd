...
The mangement team at Newark Airport know that there are a large number of 
variables that impact the running of their airport. Some they can control and 
others they can't. Understanding which variables are important and which aren't
allows management to focus their attention on problem areas with confidence and 
apply their finite resources more efficiently.

Organisations are awash with data these days. Extracting value from the data-mountain
can be a daunting task which can often lead to procrastination and a focus on 
business-as-usual (BAU) rather than change. Whilst BAU might "get the job done" 
in the short term, it doesn't facilitate growth or the adoption of new and more 
efficient business practices. 

Machine learning algorithms enables business to query large amounts of data and 
zero-in on the most crucial information within a mountain of inputs and then use 
that data to make predictions about the future based on historical patterns. 

The machine learning techniques featured in this report will allow management to
better understand their operating environment by providing statistical, fact based
evidence as to which variables have the greatest impact on the running of the airport. 

This knowledge will enable the management team at Newark Airport to take better 
informed decisions and identify areas for further research. 
...
airports. In total there were 303,748 observations with 8 variables. 

*airports* - details of 1,333 airports in the USA with 8 variables including 
their Federal Aviation Authority (FAA) airport code, latitude and longitude.

*airlines* - there are 12 airlines in this dataset with 2 variables listed
for each, the carrier name and code. 

*planes* - details of 3,521 aeroplanes with 9 variables to describe them. 
The variables include their tail number (unique identifier), year of manufacture
and seat numbers (capacity).

*weather* - weather data for all three NYC international airports in
2017. In total there are 26,201 weather observations with 15 variables. 
...
linear relationship to such a random series of independent variables. 
Their impact on delay is changeable and dependent on a myriad of combinations 
of these variables. A straight line can't be fitted to such data so trying
to predict a value for delay with a linear regression model just isn't
appropriate.
...
and the `ggplot` package in `RStudio` to generate visualisations. 

I made use of GitHub for back-up and version control as I developed this report. 
The resulting report summary and presentation was made using MS PowerPoint.
These were the tools I used during my work on the Newark Airport
assignment.\
<br> <br>

#### Descriptive Analysis

The data is historical in that all of it relates to 2017. All of the data sets 
provided are investigated in this report. When cleaning the data the summary 
statistics are reviewed for each variable before decisions to remove or impute 
missing values are taken. Similarly, plots of the data are examined to identify 
outliers and better understand what the records show and how the variables 
relate to one another. 

#### Diagnostic Analysis

Newark Airport have asked me to investigate if relationships exist between 
weather data and delays to flights. In this report I use regression analysis to 
examine the relationship between variables and to quantify that relationship 
where one exists. Uncovering such relationships will help determine what is 
causing flights to be delayed and the relative impact each of these variables 
has on departure delays. 

Beyond weather, I look at other factors which may be contributing to delays and 
develop a further model to explain why some flights may end up being cancelled. 
...
logistic regression is a categorical response. In this report models are build 
to categorise a set of variables as indicative of `DELAY` or `NO DELAY` to flight
departure times. 

Logistic regression uses a nonlinear function to model
...
between 0 and 1 for all possible combination of input values. 
<br> <br>
...
values. After which, the summary statistics reamin relatively 
unchanged for `dep_delay`.
...
For the remaining `tailnum` variable, I replace missing values with
"ABCXYZ".
...
cancelled flights (3,266) captured within `air_time`. There are now
...
Turning first to the `newark_flights_clean` data.
...
(class) in the `newark_flights_clean` dataset. We need to add one as 
logistic regression requires it. Newark Airport are interested in
...
will be excluded from quantile reduction, otherwise "10" miles will be the 
only value remaining for `visib`. For the `wind_speed` and `wind_gust` 
we will remove those values more than 1.5 times the IQR.
...
**Flight delays and Other Factors** <br> We combine `newark_flights_clean` 
...
imbalance is not massively significant so we continue.
...
We remove `air_time` and `dest` but retain `distance` which is a more
...
-   the null deviance value indicates how well the model did in predicting the 
    ...
    the value the better. This model's value is 25% lower than the weather
    model reported in the previous section. 
...
Cancelled flights can be seeen as an extension of the delayed flights issue 
at Newark Airport. As noted earlier there appear to be a number of cancelled 
flights in the `flights` dataset denoted by `NA` values within the 
`air_time` variable.
...
the *clean weather* and *clean plane* datasets to the *clean cancelled flights* 
data to create a large data set to build our cancelled model upon.
...
The class distribution of the training set will initially reflect this.
...
La Guardia has severe delays in the summer months but appears to recover  during the Christmas period of 2017. 
<br>
...
From the limited data set available, one can conclude that the weather does not 
have a significant impact on flight delays. 

Of the weather variables available and those with sufficient data, it was clear 
that only visibility could be deemed to be significant. Even then, it's inclusion 
in the model added limited, if any, predictive ability. 

Other factors outwith the weather were proven to have an impact on departure 
delays from Newark Airport. The time and day of departure were seen to have a 
significant impact on departure delays. In addition to this, the level of activity
at the airport as well as the capacity of the departing plane were also proven
to be good predictors of flight delays. 

There was also scope within the dataset to identify an extension to the problem
of delayed flights. That is, cancelled flights. Many variables were identified as 
being significant in predicting flights being cancelled. The data set for
cancelled flights was small (about 3% of the departing flights data set) which may
have resulted in the final model being less robust than one would have liked. A 
larger dataset and further work on this may provide more insight and a stronger 
predictive model. 

In terms of comparison to the other international airports in New York, Newark Airport
could be seen to have trended in-line with both JFK and La Guardia during 2017. 
...
-   Larger data set: The approach detailed above could be revisited with
    ...
    arrivals and a full complement of weather data. Data really is the key
    to better informing the machine learning algorithm which, in turn, 
...
-   Classification with more than 2 values: In each of the models
    ...
    medium delay (31 - 60 mins) and long delay (\> 60 mins). A model
